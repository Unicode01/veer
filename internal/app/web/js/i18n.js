(function () {
  ...
  app.translations = {
    'zh-CN': {
      ...
      'plugins.title': '运行时插件',
      'plugins.desc': '展示插件 manifest、对象校验、TC pipeline 链和运行时挂载状态；外部插件和数据面默认关闭。',
	  ...
	  'plugins.package.archiveHint': '选择由 veer plugin pack 生成的 .tar.gz 文件；单次最多 16 个，每个最大 32 MiB。',
      'plugins.package.archiveRequired': '请选择插件包。',
	  'plugins.package.batchLimit': '单次最多预检 16 个插件包。',
      'plugins.package.signature': '签名 sidecar（可选）',
      'plugins.package.signatureHint': '选择 veer plugin sign 生成的 .sig JSON；签名者公钥必须已加入信任列表。',
      'plugins.package.signatureInvalid': '签名 sidecar 格式无效。',
	  'plugins.package.signatureUnmatched': '签名文件无法匹配插件包；批量安装时请使用 <插件包文件名>.sig。',
	  ...
	  'plugins.package.trustedPublisherRequired': '必须可信发布者',
      ...
      'plugins.package.unsignedWarning': '此包没有可信签名，无法验证发布者身份。只应安装已经人工审查的本地包。',
      'plugins.package.unsignedBlocked': '当前安全策略要求可信签名，无法应用此未签名包。请使用受信任发布者签名，或仅在开发环境中关闭全局签名要求。',
      'plugins.package.privilegeAdditions': '新增权限',
      'plugins.package.privilegeWarning': '新版本扩大了控制面或数据面权限。摘要仅对本次预检候选有效。',
      'plugins.package.approveUnsigned': '我确认该未签名包来自可信来源，并接受无法验证发布者身份的风险。',
      'plugins.package.approvePrivileges': '我已审查并批准以上新增权限。',
	  'plugins.package.approveUnsignedBatch': '我已确认本批次中的所有未签名包均来自可信来源。',
	  'plugins.package.batchUnsignedBlocked': '当前安全策略要求可信签名，本批次包含未签名包，无法应用。',
      ...
      'plugins.trust.title': '信任密钥',
      'plugins.trust.meta': '只有信任列表中的 Ed25519 发布者才能通过签名预检。删除密钥只影响后续安装。',
      'plugins.trust.addTitle': '添加发布者',
      'plugins.trust.name': '发布者名称',
      'plugins.trust.publicKey': 'Ed25519 公钥',
      ...
      'plugins.trust.list': '已信任发布者',
      'plugins.trust.empty': '尚未添加信任密钥。',
      'plugins.trust.keyID': '密钥 ID',
      ...
      'plugins.secrets.title': '插件密钥',
      'plugins.secrets.meta': '查看插件 Secret AEAD keyring 状态并执行在线轮换；不会展示密钥材料。',
      'plugins.secrets.available': '可用',
      'plugins.secrets.persistent': '持久化',
      'plugins.secrets.activeKey': '活动密钥 ID',
      'plugins.secrets.keyCount': 'Keyring 密钥数',
      'plugins.secrets.unavailable': '插件 Secret 存储当前不可用。',
      'plugins.secrets.ephemeral': '当前密钥未持久化，不能执行安全的在线轮换。',
      'plugins.secrets.rotateHint': '轮换会重新加密所有已声明的 Secret 字段；存在未完成资源迁移时会拒绝执行。',
      'plugins.secrets.rotate': '轮换密钥',
      'plugins.secrets.rotating': '正在轮换',
      'plugins.secrets.rotateTitle': '轮换插件密钥',
      'plugins.secrets.rotateConfirm': '在线轮换插件 Secret 密钥并重新加密已有记录？',
      'plugins.secrets.rotated': '插件 Secret 密钥已轮换。',
      'plugins.secrets.rotateFailed': '轮换插件密钥失败：{{message}}',
      ...
      'search.plugins.placeholder': '搜索插件 ID、能力、对象、Hook...',
    ...
    'en-US': {
      ...
      'plugins.desc': 'Manifest, object validation, TC pipeline chain, and runtime attachment status. External plugins and dataplane are off by default.',
	  ...
	  'plugins.package.archiveHint': 'Choose up to 16 .tar.gz files produced by veer plugin pack, each up to 32 MiB.',
      ...
      'plugins.package.signature': 'Signature Sidecar (Optional)',
      'plugins.package.signatureHint': 'Choose the .sig JSON from veer plugin sign; its publisher key must already be trusted.',
      'plugins.package.signatureInvalid': 'The signature sidecar is invalid.',
	  'plugins.package.signatureUnmatched': 'A signature file could not be matched. For batches, name it <package filename>.sig.',
      ...
      'plugins.package.trusted': 'Trusted signature',
	  ...
	  'plugins.package.trustedPublisherRequired': 'Trusted Publisher Required',
      ...
      'plugins.package.unsignedWarning': 'This package has no trusted signature, so its publisher cannot be verified. Install only a locally reviewed package from a trusted source.',
      'plugins.package.unsignedBlocked': 'The active security policy requires trusted signatures, so this unsigned package cannot be applied. Sign it with a trusted publisher key or disable the global requirement only in development.',
      ...
      'plugins.package.approveUnsigned': 'I trust the source of this unsigned package and accept that its publisher identity cannot be verified.',
      'plugins.package.approvePrivileges': 'I reviewed and approve the new privileges listed above.',
	  'plugins.package.approveUnsignedBatch': 'I trust the source of every unsigned package in this batch.',
	  'plugins.package.batchUnsignedBlocked': 'The active security policy requires trusted signatures and this batch contains an unsigned package, so it cannot be applied.',
      ...
      'plugins.package.apply': 'Apply Package',
      ...
      'plugins.trust.title': 'Trust Keys',
      'plugins.trust.meta': 'Only Ed25519 publishers in this list can pass signed package staging. Removing a key affects future installs only.',
      'plugins.trust.addTitle': 'Add Publisher',
      'plugins.trust.name': 'Publisher Name',
      'plugins.trust.publicKey': 'Ed25519 Public Key',
      ...
      'plugins.trust.list': 'Trusted Publishers',
      'plugins.trust.empty': 'No trust keys have been added.',
      'plugins.trust.keyID': 'Key ID',
      ...
      'plugins.secrets.title': 'Plugin Secrets',
      'plugins.secrets.meta': 'Inspect the plugin Secret AEAD keyring and rotate it online. Key material is never displayed.',
      'plugins.secrets.available': 'Available',
      'plugins.secrets.persistent': 'Persistent',
      'plugins.secrets.activeKey': 'Active Key ID',
      'plugins.secrets.keyCount': 'Keyring Keys',
      ...
      'plugins.secrets.rotateHint': 'Rotation re-encrypts every declared Secret field and is rejected while a resource migration is pending.',
      'plugins.secrets.rotate': 'Rotate Key',
      'plugins.secrets.rotating': 'Rotating',
      'plugins.secrets.rotateTitle': 'Rotate Plugin Secret Key',
      'plugins.secrets.rotateConfirm': 'Rotate the plugin Secret key online and re-encrypt existing records?',
      'plugins.secrets.rotated': 'The plugin Secret key was rotated.',
      'plugins.secrets.rotateFailed': 'Rotating the plugin Secret key failed: {{message}}',
      ...
      'search.plugins.placeholder': 'Search plugin ID, capability, object, hook...',
