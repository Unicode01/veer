...
const (
	...
	dhcpv4MaxActiveLeases   = MaxDHCPv4PoolAddresses
	ipv4ProtocolUDP         = 17
)
...
type DHCPv4Manager struct {
	mu      sync.Mutex
	servers map[string]*managedNetworkDHCPv4Server
}

func NewDHCPv4Manager() *DHCPv4Manager {
	return &DHCPv4Manager{servers: make(map[string]*managedNetworkDHCPv4Server)}
}

func (manager *DHCPv4Manager) Ensure(config DHCPv4Config) error {
	if manager == nil {
		return nil
	}
	manager.mu.Lock()
	server := manager.servers[config.Bridge]
	if server == nil {
		server = newManagedNetworkDHCPv4Server(config)
		manager.servers[config.Bridge] = server
		server.start()
		manager.mu.Unlock()
		log.Printf("network service: dhcpv4 enabled on %s (gateway=%s pool=%s-%s dns=%v reservations=%d)", config.Bridge, config.Gateway, config.PoolStart, config.PoolEnd, config.DNSServers, len(config.Reservations))
		return nil
	}
	changed := server.update(config)
	manager.mu.Unlock()
	if changed {
		log.Printf("network service: dhcpv4 updated on %s (gateway=%s pool=%s-%s dns=%v reservations=%d)", config.Bridge, config.Gateway, config.PoolStart, config.PoolEnd, config.DNSServers, len(config.Reservations))
	}
	return nil
}

func (manager *DHCPv4Manager) Delete(bridge string) error {
	if manager == nil {
		return nil
	}
	manager.mu.Lock()
	server := manager.servers[bridge]
	delete(manager.servers, bridge)
	manager.mu.Unlock()
	if server != nil {
		server.stop()
	}
...
func (manager *DHCPv4Manager) Snapshot() map[string]DHCPv4RuntimeState {
	if manager == nil {
		return nil
	}
	manager.mu.Lock()
	servers := make(map[string]*managedNetworkDHCPv4Server, len(manager.servers))
	for bridge, server := range manager.servers {
	...
	states := make(map[string]DHCPv4RuntimeState, len(servers))
...
func (manager *DHCPv4Manager) Close() error {
	if manager == nil {
		return nil
	}
	manager.mu.Lock()
	servers := manager.servers
	manager.servers = make(map[string]*managedNetworkDHCPv4Server)
	manager.mu.Unlock()
...
func loadManagedNetworkDHCPv4InterfaceInfos() ([]InterfaceInfo, error) {
	...
	return netinfo.LoadInterfaceInfos()
}
