...
#ifndef BPF_FUNC_skb_load_bytes
...
#ifndef BPF_F_ADJ_ROOM_NO_CSUM_RESET
...
struct pppoe_tunnel_config {
	__u32 enabled;
	__u32 local_ifindex;
	__u32 pipeline_ifindex;
	__u32 wan_ifindex;
	__u16 session_id;
	__u16 flags;
	__u16 mss_clamp_v4;
	__u16 mss_clamp_v6;
	__u8 local_src_mac[6];
...
static int (*const bpf_skb_load_bytes)(struct __sk_buff *skb, __u32 offset, void *to, __u32 len) = (void *)BPF_FUNC_skb_load_bytes;
...
struct bpf_map_def SEC("maps") pppoe_traffic_stats = {
	.type = BPF_MAP_TYPE_PERCPU_ARRAY,
	.key_size = sizeof(__u32),
	.value_size = sizeof(struct pppoe_traffic_stats),
	.max_entries = 1,
};
...
static __always_inline void bump_traffic_stats(int receive, __u16 bytes)
{
	__u32 key = 0;
	struct pppoe_traffic_stats *stats = bpf_map_lookup_elem(&pppoe_traffic_stats, &key);
...
static __always_inline int encap_known_l3_to_pppoe(struct __sk_buff *skb, struct pppoe_tunnel_config *cfg, __u16 l3_len, __u16 ppp_proto)
{
	...
	if (bpf_skb_adjust_room(skb, sizeof(hdr), BPF_ADJ_ROOM_MAC, PPPOE_ENCAP_ADJ_FLAGS) < 0) {
		bump_tunnel_stat(17);
		return TC_ACT_SHOT;
	}
...
static __always_inline int decap_pppoe_to_l3(struct __sk_buff *skb, struct pppoe_tunnel_config *cfg)
{
	...
	if ((void *)(eth + 1) > data_end || eth->h_proto != htons16(ETH_P_PPP_SES))
		return TC_ACT_UNSPEC;
	if (!mac_addr_eq(eth->h_dest, cfg->wan_src_mac) ||
		!mac_addr_eq(eth->h_source, cfg->wan_dst_mac))
		return TC_ACT_UNSPEC;
	bump_tunnel_stat(3);
	packet_hdr = data + 14;
	if ((void *)(packet_hdr + 1) > data_end)
	...
	if (store_eth(skb, cfg->local_dst_mac, cfg->local_src_mac, eth_proto) < 0) {
		bump_tunnel_stat(6);
		return TC_ACT_SHOT;
	}
...
int tc_tunnel(struct __sk_buff *skb)
{
	...
	if (!cfg || !cfg->enabled)
		goto next;
	if (skb_matches_ifindex(skb, cfg->pipeline_ifindex)) {
		bump_tunnel_stat(1);
		act = encap_l3_to_pppoe(skb, cfg);
		if (act != TC_ACT_UNSPEC)
			return act;
	} else if (skb_matches_ifindex(skb, cfg->wan_ifindex)) {
		bump_tunnel_stat(2);
