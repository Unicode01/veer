...
find_python() {
	...
	echo "python3 or python is required to verify plugin manifests" >&2
	exit 1
}

PYTHON_BIN=$(find_python)

"$PYTHON_BIN" - "$PLUGIN_SOURCE_DIR" <<'PY'
import hashlib
import json
import pathlib
import re
...
def verify_control(manifest_path, plugin_dir, manifest, stability):
    ...
    if ("net.admin" in permission_set or "net.l2" in permission_set or "net.tcp" in permission_set or "net.udp" in permission_set) and not net_access:
        add_error(manifest_path, "control.net_access is required when net.admin, net.l2, net.tcp, or net.udp permission is declared")
    seen_net_access = set()
    for index, item in enumerate(net_access):
        ...
        for operation in operations:
            ...
            if operation == "tcp" and "net.tcp" not in permission_set:
                add_error(manifest_path, f"{label}.operation {operation!r} requires net.tcp permission")
            if operation not in {"l2", "tcp", "udp"} and "net.admin" not in permission_set:
...
if not root.is_dir():
    raise SystemExit(f"plugin directory does not exist: {root}")
...
if checked == 0:
    errors.append(f"no plugin manifests found under {root}")
...
print(f"verified {checked} plugin manifest(s)")
PY
