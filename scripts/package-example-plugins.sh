...
EXAMPLE_DIR="$ROOT_DIR/examples/plugins"
OUT_DIR=${FORWARD_PLUGIN_PACKAGE_DIR:-"$ROOT_DIR/dist/plugins-runtime"}
...
write_plugin_list() {
	out_file=$1
	"$PYTHON_BIN" - "$EXAMPLE_DIR" "$PACKAGE_STABILITY" >"$out_file" <<'PY'
import json
import pathlib
import sys
...
PY
	if [ ! -s "$out_file" ]; then
		echo "no example plugins match FORWARD_PLUGIN_PACKAGE_STABILITY=$PACKAGE_STABILITY" >&2
		exit 1
	fi
}

case "$OUT_DIR" in
	""|"/"|"$ROOT_DIR"|"$ROOT_DIR/"|"$EXAMPLE_DIR"|"$EXAMPLE_DIR/")
...
if [ "${FORWARD_PLUGIN_PACKAGE_SKIP_BUILD:-0}" != "1" ]; then
	...
	while IFS= read -r plugin_name; do
		...
		plugin_dir="$EXAMPLE_DIR/$plugin_name"
...
echo "packaged example plugins into $OUT_DIR (stability=$PACKAGE_STABILITY)"
