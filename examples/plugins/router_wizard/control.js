...
function compactRecord(record) {
  ...
  return {
    ...
    interface: text(data.interface || data.wan_interface || data.host_interface || data.bridge || data.out_interface || ''),
...
function storedConfig() {
  try {
    var record = resources.get(CONFIG_RESOURCE, CONFIG_KEY);
...
function normalizeConfig(raw) {
  ...
  return {
    wan: {
      ...
      host_interface: ifaceName(wan.host_interface || raw.wan_host_interface || 'fwdlocal0', 'WAN host interface'),
      vtap_interface: ifaceName(wan.vtap_interface || raw.wan_vtap_interface || 'fwdvtap0', 'WAN vtap interface'),
      mtu: intValue(wan.mtu || raw.wan_mtu || raw.mru, 576, 65535, 1492),
      pppoe: {
        interface: optionalIfaceName(wan.pppoe_interface || raw.pppoe_interface || raw.interface || ''),
        username: text(wan.username || raw.pppoe_username || raw.username || ''),
        password: text(wan.password || raw.pppoe_password || raw.password || ''),
        service: text(wan.service || raw.pppoe_service || raw.service || ''),
        auth: normalizeAuth(wan.auth || raw.pppoe_auth || raw.auth || 'pap'),
        negotiate_ipv6: bool(firstDefined(wan.negotiate_ipv6, raw.pppoe_negotiate_ipv6), false),
        request_pd: bool(firstDefined(wan.request_pd, raw.pppoe_request_pd), false),
        install_tunnel: bool(firstDefined(wan.install_tunnel, raw.pppoe_install_tunnel), true),
        auto_redial: bool(firstDefined(wan.auto_redial, raw.pppoe_auto_redial), true),
        keepalive_interval_ms: intValue(wan.keepalive_interval_ms || raw.pppoe_keepalive_interval_ms, 0, 86400000, 5000)
      }
    },
    lan: {
...
function pppoePayload(cfg) {
  ...
  return {
    ...
    coupled: true,
    lan_interface: cfg.wan.vtap_interface,
    lan_peer_interface: cfg.wan.host_interface,
    wan_core_sync: true,
    wan_core_apply: true,
...
function wanCorePayload(cfg) {
  return {
    ...
    wan_interface: cfg.wan.egress_interface || cfg.wan.pppoe.interface,
    mtu: cfg.wan.mtu,
    host_interface: cfg.wan.host_interface,
    vtap_interface: cfg.wan.vtap_interface,
    egress_nat_redirect_mode: cfg.lan.redirect_mode
  };
}
...
function lanPayload(cfg) {
  ...
  return {
    ...
    redirect_mode: cfg.lan.redirect_mode
  };
}
...
function effectiveWANInterface(cfg, wanStatus, pppoeLink) {
  ...
  return text(wanData.egress_nat_parent_interface || forwardCore.egress_nat_interface ||
    ...
    wanData.host_interface || linkData.host_interface || linkData.wan_interface || '');
}
