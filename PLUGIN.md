...
仓库的 `plugins/` 目录存放随项目维护的运行时插件。Forward 默认扫描 `plugins/<plugin_id>/`，每个插件使用独立子目录。
...
pipeline.node({
  id: "my_plugin",
  description: "Optional logical node shown in the fvtap pipeline."
});
...
- `pipeline.node({id, description})`：声明纯 fvtap 逻辑链节点，不创建 Linux 接口。
- `pipeline.handoff({id, type, description})`：声明系统接入适配器，通常对应 bridge、veth、tun 或本机路由管理。
- `pipeline.attach({direction, attach, priority, program, interfaces})`：声明 eBPF 程序应进入哪条 fvtap 逻辑链和 TC ingress/egress 挂载点；Forward 根据 core priority 自动编排 core 前后和 tail-call slot。
...
Forward 每 2 秒检查插件源目录内容指纹。插件增删或 `plugin.json`、`control.js`、UI、eBPF object 变化只会设置 `update_available`，不会自动执行候选代码或重建数据面；WebUI 的“应用更新”或 `POST /api/plugins/reload` 会复制一份稳定候选快照，完成 manifest、control、UI 和 object 校验后再 reconcile。校验或运行时切换失败会保留上一份已应用快照。常规文件使用受限 SHA256 内容 hash，超大文件只纳入路径、大小和 mtime 等元数据。
...
切换时 Forward 会暂停该插件的新调用，排空主 VM 与命名 worker 的在途请求，在旧 VM 调用 `onUpgradeSnapshot`，再在候选 VM 调用 `onUpgradeRestore`；全部成功后才原子替换 VM 所有权。失败会丢弃候选并继续使用旧 VM。旧版本实现 snapshot 而候选缺少 restore 时会拒绝更新；候选首次加入 restore、旧版本没有 snapshot 时收到的 `state` 为 `null`；两端都没有钩子时保持冷替换语义。
...
实现 restore 后，宿主持有的 timer 会保持原 generation 和触发时间，socket 句柄会在候选仍声明相同 `net.tcp`/`net.udp` 权限及 `net_access` 时转移，现有命名 worker 也会按 `scope: "worker"` 独立迁移。宿主 core 重建时，Forward 仅会在插件 ID、object ID 和实际加载字节的 SHA256 都不变时复用规格兼容的插件私有 map；object 内容升级、禁用或删除不会继承这些 map。eBPF map 中需要跨 object 更新保留的协议状态仍应落入 `runtime_apply` resource；Forward 会在新 dataplane 挂载后重放这类资源。VM snapshot 不替代 dataplane map 的兼容设计。
...
4. 插件页面不能拿到 Web Token；读写本插件数据时调用 `ForwardPluginHost.data.*` 或 `ForwardPluginHost.action()`。
...
- `ForwardPluginHost.plugins.resources.list(plugin, resource, {limit, offset})`
- `ForwardPluginHost.plugins.resources.get(plugin, resource, key)`
...
宿主还注入基础组件和 class 名，例如 `host.card()`、`host.button()`、`host.table()`、`host.recordPicker()`、`host.collectionEditor()`、`fwd-card`、`fwd-button`、`fwd-table`。`recordPicker` 用于已有记录/新建记录切换，`collectionEditor` 用于路由等结构化数组，避免页面要求用户手写内部 key 或原始 JSON。
...
启用 `plugins_dataplane_enabled=true` 后，使用 `pipeline.attach()` 注册 TC `direction=forward` 或 `direction=reply` 的可信插件可以进入内置 `fvtap` pipeline。默认没有实际插件链时，不会给热路径增加额外 lookup。

`fvtap` 是逻辑 pipeline，不是 Linux netdev。它的作用是把宿主 TC dispatcher、插件 eBPF 程序和内置 forward core 编排成一条 tail-call chain：
...
- `wan_core`、`vtolocal` 在需要接入 Linux 协议栈、本机 socket 或系统路由时只创建一个 dummy 边界；不再为逻辑 fvtap 创建 veth peer。

因此，PPPoE、firewall、VXLAN 这类数据面插件应优先实现为逻辑 chain。只有需要让 Linux 本机看见 L3 边界时才创建 dummy/tun/bridge 等系统接口；插件节点本身始终不占用 netdev。

推荐写法是让 Goja 声明意图，具体 pre/post stage、prog-array slot 和 core 插入点由 Forward runtime 决定：
...
PPPoE 单接口路径使用 `local0 TC egress -> PPPoE stage -> physical WAN` 和 `physical WAN TC ingress -> PPPoE stage -> local0 ingress`。`local0` 是唯一新增的 Linux 接口；PPPoE、fvtap 和各插件 Hook 都只是逻辑节点。
...
- `packet_observer`：TC pipeline 观测插件，需要执行 `build.sh` 生成 eBPF object；仍是 `lab`。
- `wan_core`：协议中立 WAN handoff 插件，消费标准化 session，并管理一个本地 L3 dummy 边界。
- `lan_core`：LAN bridge 和 synthetic Egress NAT plan 插件。
- `vtolocal`：创建一个本地 L3 dummy，并管理该接口上的地址与路由。
